    ...
    if (isset($_POST['action'])) {
        ...
        switch ($_POST['action']) {
            case 'create_contract':
                // Create REAL DOCX contract file
                ...
                // Create DOCX properly using PhpWord
                $phpWord = new \PhpOffice\PhpWord\PhpWord();

                $section = $phpWord->addSection();

                $section->addTitle('Test Contract Agreement', 1);

                $section->addText(
                    'This is a test contract document for digital signature testing.',
                    ['name' => 'Arial', 'size' => 12]
                );

                $section->addTextBreak(1);

                $section->addText('Contract Terms:', ['bold' => true]);

                $section->addListItem('This is a test agreement');
                $section->addListItem('Valid for testing purposes only');
                $section->addListItem('Generated at: ' . date('Y-m-d H:i:s'));

                $section->addTextBreak(2);

                $section->addText('Authorized Signature: _____________________');
                ...
                $sql = "INSERT INTO contracts 
                        (id, title, description, file_path, signing_state, created_by) 
                        VALUES 
                        (
                            1,
                            'Test Contract',
                            'This is a test contract for signature verification',
                            :file_path,
                            'DRAFT',
                            'test@user.com'
                        )";
                ...
                $message = "Valid DOCX contract created successfully!";
